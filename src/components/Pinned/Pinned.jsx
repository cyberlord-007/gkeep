...
const Pinned = ({ mode, notes, pinNotes, archiveNotes, deleteNotes }) => {
  ...
  const [clickedCard, setClickedCard] = useState({
    ...JSON.parse(localStorage.getItem('pinnedNote')),
  });
      ...
      setPinnedNotes(
        Object.fromEntries(
          Object.entries(notes).filter(
            ([noteDoc, note]) =>
              note?.pinned === true && note?.archived === false
          )
        )
      );
  }, [notes]);
      ...
      <NotesContainer mode={mode}>
        ...
        <NotesWrapper>
          {pinnedNotes && Object.keys(pinnedNotes).length === 0 ? (
            <PageError>
              <FaExclamationCircle
                size={window.screen.width < 480 ? '25' : '40'}
                color={mode ? '#dad6d6' : '#7c7979'}
              />
              <ErrorText mode={mode}>No pinned notes found</ErrorText>
            </PageError>
          ) : (
            pinnedNotes &&
            Object.entries(pinnedNotes).map(([noteDoc, note], idx) => (
              <NotesCard
                onClick={() => handleClick(note, noteDoc)}
                show={note}
                key={note?.title}
              >
                <CardHeader>
                ...
                <GreyLine></GreyLine>
                <CardBody>
                  <NoteDesc mode={mode}>{note?.body}</NoteDesc>
                </CardBody>
                <Actions>
                  <IconWrap mode={mode}>
                    {note?.pinned ? (
